...
#include<linux/kernel.h>  		// To print in logs using KERN_INFO		 
#include<linux/usb.h>     		
...
#include <linux/blkdev.h> 		 //While dealing with block devices
#include <linux/errno.h>   		
#include<linux/workqueue.h>  		

////////////////// Defining vid and Pid of pendrives experimented //////////////////////

#define SAN_VID  0x0781			//Sandisk vid and pid
#define SAN_PID  0x556b

#define TB_VID  0x0930			//Toshiba vid and pid
#define TB_PID  0x6544


#define GET_MAX_LUN        0xFE    		//code for getting maximum lun
#define REQ_TYPE	   0xA1 	     	//code for request
#define RESET_TYPE         0x21    		//code for reset
#define RESET_D			   0xFF    	//code to tell 

/////////////   Defining capacity, inquiry commands ///////////////////////////////////

#define READ_CAPACITY_LENGTH          0x08  
#define READ_INQ_LENGTH          0x24     
#define READ_DATA               0X200     //simply to storw sector length


/////////// Number of bytes for cbw and csw required ///////////////////////////////////
...
/* The internal structure representation of the Device */
static struct rb_device
{
					
	unsigned int size;		// size of the device (in sectors) 
	spinlock_t lock;		//exclusive access to  request queue
	struct request_queue *rb_queue; // request queue 
	struct gendisk *rb_disk;
} rb_dev;


static u32 tag = 1;       //to generate unique tag
static u_int rb_major = 0;     //to get major number for block device
...
r=usb_control_msg(udev, usb_rcvctrlpipe (udev,0x00),GET_MAX_LUN,REQ_TYPE, 0, 0, lun1, 1, 10000);  
memcpy(&lun, lun1, 1);
kfree(lun1);
printk("----------------------------\n");
...
int only_inq()
{
	...
	int i,k;			 
	u8 cdb_len;         
	int size;  //to store recived size 
...
//---------------- For only capacity determination----------------------//
...
int only_read(sector_t sector_off, u8 *buffer, unsigned int sectors)
{
	unsigned int hi;         //variable includes total bytes recived, bulk send , bulk recieve, stauts, received sizes, CBWCB length
	int i,r,k,r1;			
	int size2,size3;  
	u8 cdb_len;          
	int ct=0;   //incase of stall 
...
static void rb_transfer(struct work_struct *work)
{
	...
	unsigned int sectors;    // Represents number of sectors
	u8 *buffer;   //where to pass fetched data or wrting data is pointed by this 
...
static void rb_request(struct request_queue *q)
{
	...
	while ((req = blk_fetch_request(q)) != NULL)
	{
			...
			working->data = req;   			//store arguements for accessing in bottom half function
...
//////////////////////  Initilize as block device  //////////////////////////

int initblock()
{
	  //maximum logical block adress and block size
	rb_dev.size =max_lba+1; 
	...
	myqueue = create_workqueue("myqueue"); //creating work queue
	

 	/* Setting the major number, minor number and other device operations */
	...
	printk(KERN_INFO "Info: Pendrive- Block driver initialised (%d sectors; %d bytes)\n",
...
static int usbdev_probe(struct usb_interface *interface, const struct usb_device_id *id)
{
	...
	if(id->idProduct == SAN_PID)
	{
		printk(KERN_INFO "Known USB drivedetected\n");
		printk(KERN_INFO "San Disk Pendrive plugged in\n");
	}
	else if(id->idProduct == TB_PID)
	{
		printk(KERN_INFO "Known USB drivedetected\n");
		printk(KERN_INFO "Toshiba plugged in\n");
	}
...
/*Device Regarding operations*/
static struct usb_driver usbdev_driver = {
	name: "usbdev",  		  //name of the device
	probe: usbdev_probe,		 // When Device is plugged in
	disconnect: usbdev_disconnect,   // When disconnect a device
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("MITILESH-2019H1400139H");
